...
- Set `cache-dependency-path` to a repository-relative dependency file when the default lockfile or requirements file is elsewhere. Defaults are resolved under `package-path`, so nested projects use keys based on their own `uv.lock`, `poetry.lock`, `requirements-txt`, `package-lock.json`, `pnpm-lock.yaml`, or `yarn.lock`.
